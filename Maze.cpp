...
Maze::Maze() {
    ...
    #ifdef __clang__
    // Suppress unused variable warning specific to Clang
        (void)build_x;
        (void)build_y;
        (void)build_z;
        (void)x;
...
void Maze::readMazeTerminal(Loc& loc,mcpp::MinecraftConnection& mc){
    ...
    build_x = mc.getPlayerPosition().x;
    build_y = mc.getPlayerPosition().y;
    build_z = mc.getPlayerPosition().z;
    // this->CoordinateInput(build_x, build_y, build_z,mc);
    loc.setCoordinates(build_x, build_y, build_z);

    std::cout << "Enter the length and width of maze: " << std::endl;
    this->getNumericInput(Maze_Length, Maze_Width);
    loc.setMazeSize(Maze_Length, Maze_Width);
    ...
    loc.setLocElement(Maze_Structure);

    std::cout << "Maze read successfully" << std::endl;
    std::cout << "**Printing Maze**" << std::endl;
    std::cout << "BasePoint: (" << loc.getX_Coord() << ", " << loc.getY_Coord() << ", " << loc.getZ_Coord() << ")" << std::endl;
    std::cout << "Structure: " << std::endl;

    this->printMazeStructure(loc.getLocStructure(), loc.getLength(), loc.getWidth());
...
void Maze::FirstStep(int Maze_Length, int Maze_Width, char**& Maze_Structure, int& startx, int& startz,  bool mode, Loc& loc){
    int min = 1; //length  min range
    int max = Maze_Length - 2;  // length max range

    int min2 = 1;//width  min range
    int max2 = Maze_Width - 2; //width max range
    //this is done to ensure that the first step is made inside the maze boundaries and not on it.


    std::random_device r;
    std::default_random_engine engine(r()); //seeding the engine
    std::uniform_int_distribution<int> uniform_dist(min, max); //setting the range for length
    std::uniform_int_distribution<int> uniform_dist2(min2, max2);// setting the range for width
    int value = -1;
    int value2 = -1;
    int exitLoop = 0;

    while(exitLoop == 0){ //gets coordinates (value, value2) where both are odd.
    ...
    int arr[4] = {(Maze_Length - 1) - value, value, (Maze_Width - 1) - value2, value2};
    ...
    if(!mode){ //normal mode    
        ...
        if(minimum_index == 0){//bottom
            Maze_Structure[Maze_Length - 1][value2] = '.';
            exitX = Maze_Length-1 + 1;
            exitZ = value2;
        }
        else if(minimum_index == 1){//top
            Maze_Structure[0][value2] = '.';
            exitX = 0 -1;
            exitZ = value2;
        }
        else if(minimum_index == 2){//right
            Maze_Structure[value][Maze_Width - 1] = '.';
            exitX = value;
            exitZ = Maze_Width-1+1;
        }
        else if(minimum_index == 3){//left
            Maze_Structure[value][0] = '.';
            exitX = value;
            exitZ = 0 -1;
        }

        exitX = loc.getX_Coord() + exitX;
        exitZ = loc.getZ_Coord() + exitZ;

        std::cout << "endPoint: (" << exitX<< " , "<<exitZ<< ")" << std::endl;
    }
    else{//testing
    // std::cout << "debug" << std::endl;
        Maze_Structure[1][1] = '*'; //start point always at (1,1)
        startx = 1;
        startz = 1;
        Maze_Structure[1][0] = '.'; //end point alway at (1,0)
    }
}
...
void Maze::makeRandomMaze(Loc& loc,mcpp::MinecraftConnection& mc, bool mode) {
    int build_x = 0;
    int build_y = 0;
    int build_z = 0;
    int Maze_Length = 0;
    ...
    int number = 0;
            this->FirstStep(l,w,Maze_Structure3,startx,startz,mode, loc);
        ...
        loc.setLocElement(Maze_Structure3);
